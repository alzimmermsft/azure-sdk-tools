...
public class MissingJavadocCodeSnippetsRule implements DiagnosticRule {
    ...
    private static final Pattern NEW_CODESNIPPET_TAG = Pattern.compile(
        "(\\s*)\\*?\\s*<!--\\s+src_embed\\s+([a-zA-Z0-9.#\\-_]+)\\s*-->");
    ...
    public void scanIndividual(final CompilationUnit cu, final APIListing listing) {
        getClasses(cu).forEach(typeDeclaration -> {
            ...
            if (typeDeclaration.getJavadocComment().isPresent() && (className.endsWith("Client") || className.endsWith(
                "Builder"))) {

                if (!javadocContainsCodeSnippetTag(typeDeclaration.getJavadocComment().get().getContent())) {
                    listing.addDiagnostic((new Diagnostic(INFO, makeId(typeDeclaration),
            ...
            if (className.endsWith("Client")) {
                ...
                boolean serviceMethodHasCodesnippets = getPublicOrProtectedMethods(cu).filter(
                        methodDeclaration -> methodDeclaration.isAnnotationPresent("ServiceMethod"))
